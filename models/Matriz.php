...
class Matriz {
    ...
    // NUEVA FUNCIÓN: Trae solo los equipos asignados a un usuario específico (Para el Rol 3)
    public function obtenerDatosPorColaborador($id_usuario) {
        $query = "SELECT 
                    a.id_activo,
                    a.codigo_interno,
                    c.nombre as equipo,          
                  ...
                  FROM activos a
                  INNER JOIN categorias c ON a.id_categoria = c.id_categoria
                  INNER JOIN usuarios u ON a.id_usuario_responsable = u.id_usuario
                  LEFT JOIN sedes s ON a.id_sede_actual = s.id_sede
                  LEFT JOIN (
                        SELECT id_activo, MAX(fecha_asignacion) as fecha_asignacion
                        FROM historial_movimientos
    ...
    // EL CAMBIO ESTÁ AQUÍ: Solo trae las sedes que están con estado 'Activo'
    public function obtenerSedes() {
        $query = "SELECT * FROM sedes WHERE estado = 'Activo' ORDER BY nombre ASC";
        $stmt = $this->conn->prepare($query);
        $stmt->execute();
        return $stmt;
    }

    // Para llenar el select de "Nuevo Responsable"
